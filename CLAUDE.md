...
- **Meeting-type classificatie robuuster + eerlijke Insights-melding** (`92f20ba`, 5 juli, gevonden door daily-code-review 4 juli): `_detect_meeting_type()` classificeerde eerder een klantgesprek stilzwijgend als `internal` zodra Jamie geen e-mail aan een deelnemer koppelde (gebeurde bij Lead3, 3 juli). Onbekende deelnemers blokkeren nu de interne classificatie. Daarnaast toont de Insights-reply-handler in `slack_app.py` niet langer onterecht "✅" als een placeholder-vervanging in de Meetingnote mislukt (`_insights_update_message()`). 9 nieuwe tests, suite 51 groen.
...
> Als je in een Claude Code sessie de MCP Drive-connector (bv. `mcp__…__search_files`, `list_recent_files`) gebruikt en je krijgt een **lege lijst, alleen een `nextPageToken`, of "niet gevonden"** op iets in de Shared Drive: **dit is de bekende bug, NIET de werkelijkheid.** De connector geeft stille valse negatieven bij zowel mapopvraging (`parentId`) als zoeken (`fullText`/`title`). Mechanisme: submappen worden getoond, bestanden verborgen — een map lijkt dus alleen "leeg" als hij geen submappen heeft, en een map die submappen toont kan alsnog al zijn bestanden verbergen. Ook "map bevestigd leeg" en spookresultaten uit oude/andere Drives horen bij dit patroon.
> **Doe dan dit, in deze volgorde:** (1) concludeer NOOIT dat iets afwezig/leeg/niet-gemigreerd is op basis van een connector-uitkomst; (2) lezen op een bekend file-ID werkt wél; (3) voor grondwaarheid: draai het serviceaccount-script zélf via SSH naar de VM (bewezen werkend vanuit lokale sessies): `ssh thomas@35.253.206.86 'cd ~/Ainstein && python3 scripts/verify_shared_drive.py [folderId]'`. Dit is de betrouwbare lezer en onderbreekt Thomas niet. Werkt SSH niet (bv. remote/web-sessie zonder lokale config), vraag dan Thomas het te draaien; (4) start GEEN groot onderzoek dat "ontbreekt/incompleet" concludeert. Dit is drie keer eerder fout gegaan en kostte een keer een volledig multi-agent onderzoek op besmette data. Zie `memory/connector_access_paths.md`.
>
> **Read-only grens:** SSH naar de VM mag alleen voor read-only verificatie (het verify-script of lezen). NOOIT schrijven, herstarten, of deployen vanuit een sessie via SSH — deploys lopen altijd via GitHub Actions (push naar `main`). Dit is de enige uitzondering op "Thomas raakt de VM nooit handmatig aan": alleen lezen, alleen om de connector-blokkade te omzeilen.
