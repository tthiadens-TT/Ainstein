...
Standaard: medium.com/@minkowskispectacles, alle beschikbare artikelen.
...
log = logging.getLogger("scrape_medium")
...
def _parse_rss(data: bytes) -> list[dict]:
    ...
    for item in channel.findall("item"):
        ...
        # Strip Medium's CDATA paywall notice if present
...
def _article_to_md(article: dict) -> str:
    ...
    author_line = f"_Auteur: {article['author']}_\n" if article["author"] else ""
    lines = [
        f"## {article['title']}",
        f"_Gepubliceerd: {date_str}_",
        author_line + f"_URL: {article['url']}_",
        "",
        article["content"],
        "",
        "---",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Medium RSS → Drive markdown bakje")
    ...
    log.info("=== Medium scraper gestart ===")
    ...
    for year, arts in sorted(groups.items()):
        doc = _build_year_doc(year, arts, args.url)
        filename = f"medium_minkowski_{year}"
