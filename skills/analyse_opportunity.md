...
## Output Structuur (acquisitieprotocol)

Wanneer de analyse wordt gebruikt voor acquisitie (niet alleen voor interne beoordeling), volg dan de 7-stappen structuur uit `acquisitie_protocol.md`:

1. **ICP-check** — verdict Fit / Fit met voorbehoud / Geen fit
2. **DMU-analyse** — beslisser, beïnvloeder, poortwachter, eerste ingang + waarom
3. **Propositie-koppeling** — 3 verbindingen, letterlijke citaten, gekoppeld aan message pillar
4. **Concurrentierisico** — met aanname-labels
5. **Sales-methodiek** — keuze + onderbouwing + eerste concrete stap
6. **Timing** — dag 0 / dag 10 / dag 21
7. **Eerste bericht** — max 100 woorden + compacte versie 70 woorden, concrete tijdsuggestie (nooit "Zou je open staan voor een gesprek?")
8. **Verificatielijst** — wat Jörgen moet doen vóór verzending
...
- Every first-contact message ends with a concrete time suggestion, never an open question.
- Label all unverified claims as: [aanname — niet extern geverifieerd]

## Quality Check
Before finalizing, ask:
- Have I made the opportunity clearer, sharper, and easier to act on?
- Does the first-contact message end with a concrete time suggestion (not "Zou je open staan voor een gesprek?")?
- Is the verification list complete (channel active? reference available? prior contact checked?)?
